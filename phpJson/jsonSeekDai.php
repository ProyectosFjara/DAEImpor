...
*/
	...
	function loadListNoProgramado (){
		...
		$str_SQL = "EXEC  $nameBase..MON_SELECT_TRAMITES_DIGITADOR_EXP  '$oficina'"; //print $str_SQL;
		...
		if ( $str_SQL != ''){
			...
			while ($row  =  $conn->db_fetch_array( $rs )){
				...
				$totalTimeBandeja = $class_fn_basic->sysGetDataFieldSrv( $row[ "totalTimeBandeja" ] );
				...
				/**se realizó el cambio porque realizaba el conteo unicamente si esta en las etapas ***/
		...
		$arr_paging = array('SQL'=>$str_SQL, 'contOtraEtapa'=>$contAsigOtraEtapa, 'totalCount'=>$contAsig,'totalCountDae'=>$contDae,'totalTimeBandeja'=>$totalTimeBandeja, 'data'=> $arr_data, 'conexion'=>'');
		...
		$str_SQL = 	" $nameBase..MON_SELECT_DIGITADOR_EXP_LIST ";//'$oficina'
			...
			if ( $str_SQL != ''){
				...
				while ($row  =  $conn->db_fetch_array( $rs ) ){
					...
					$trmRevFinal= $class_fn_basic->sysGetDataFieldSrv( $row[ "trmRevFinal" ] );
					array_push($arr_data,array(
	...
	function loadOperacionesUlt (){
		...
		$str_SQL = 	" $nameBase..MON_SELECT_LIST_TRM_DIG_EXP '$digitadorId'"; //print $str_SQL;
		$cont = 0;
		if ( $str_SQL != ''){
			$rs  =  $conn->db_query( $str_SQL );
			while ($row  =  $conn->db_fetch_array( $rs ) ){
				...
				$tiempoTotal = $class_fn_basic->sysGetDataFieldSrv( $row[ "tiempoTotal" ] );
				$tiempoTotalfechas = $class_fn_basic->sysGetDataFieldSrv( $row[ "tiempoTotalfechas" ] );
				...
				array_push($arr_data,array(
					...
					'tiempoTotalfechas'=>$tiempoTotalfechas
				));
			}
		}
